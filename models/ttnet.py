...
from tensorflow.keras.layers import (Conv2D, ReLU, Flatten, Dense, 
                                     BatchNormalization, MaxPool2D, Dropout, Conv2DTranspose)
from tensorflow.keras import (Input, Model)

def conv_block(layer, filters, maxpool=True):
    """The ConvBlock in TTNet"""
    x = Conv2D(filters=filters, kernel_size=(3,3), strides=1, padding="same")(layer)
...
def ball_detection(input, dropout):
    ...
    conv1 = Conv2D(filters=64, kernel_size=(1,1), strides=1, padding="same")(input) 
    ...
    fc2 = Dense(units=896, activation="linear")(drop4)
    relu3 = ReLU()(fc2)
    drop5 = Dropout(rate=dropout)(relu3)
    fc3 = Dense(units=448, activation="sigmoid")(drop5)
    return fc3, block2, block3, block4, block5, block6

def deconv_block(layer, filters):
    """The DeconvBlock in TTNet"""
    x = Conv2D(filters=filters/2, kernel_size=(1,1), strides=(1,1),padding="valid")(layer)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = Conv2DTranspose(filters=filters/2, kernel_size=(3,3), strides=(2,2), padding="same")(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = Conv2D(filters=filters, kernel_size=(1,1), strides=(1,1),padding="valid")(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)        
    return x
...
def sem_segmentation(inputs):
    ...
    tconv1 = Conv2DTranspose(filters=32, kernel_size=(3,3), strides=(2,2),padding="valid")(dblock2)
    relu1 = ReLU()(tconv1)
    conv1 = Conv2D(filters=32, kernel_size=(3,3), strides=(2,2))(relu1)
    relu2 = ReLU()(conv1)
    conv2 = Conv2D(filters=3, kernel_size=(2,2), strides=(2,2), activation="sigmoid")(relu2)
    return conv2

def event_spotting(input, dropout):
    """Event spotting stage."""
    conv1 = Conv2D(filters=64, kernel_size=(1,1))(input)
    bnorm1 = BatchNormalization()(conv1)
    relu1 = ReLU()(bnorm1)
